...
const DOCUMENT_CONFIG = {
  photo: {
    ...
    multi: false,
    tooltip:
      "Upload one recent passport-size photograph with a white background (JPG, JPEG, or PNG). The tool creates a JPG/JPEG photo at 100–200 KB, 200 DPI, and 700 × 900 px (3.5:4.5 aspect ratio).",
  },
  signature: {
  ...
  citizenship: {
    label:
      "Upload your scanned English Translated Citizenship certified by Public Notary:*",
    name: "citizenship",
...
export default function CompexDocumentsTool() {
  ...
  const fileInputs = useRef(
    Object.fromEntries(Object.keys(DOCUMENT_CONFIG).map((k) => [k, null])),
  );
  ...
  const handlePhotoCropComplete = async (croppedAreaPixels) => {
    ...
    try {
      ...
      URL.revokeObjectURL(photoCrop.imageSrc);
      setPhotoValidationError(
        "The photo could not be cropped. Please try another JPG/JPEG/PNG file.",
      );
      setPhotoCrop(null);
    }
  };
    ...
    try {
      ...
      if (cfg.multi) {
        ...
        for (const img of imageFiles) {
          const compressedBlob = await compressImage(img, {
            maxSizeMB: perImageSizeMB,
          });
          compressedImageFiles.push(
            new File([compressedBlob], img.name, { type: "image/jpeg" }),
          );
        }

        const merged = await imagesToPdf(compressedImageFiles);
        finalFile = new File([merged], `${cfg.name}.pdf`, {
        ...
        finalFile = new File([finalFile], `${cfg.name}.jpg`, {
          type: "image/jpeg",
        });
      }
      ...
      if (key !== "photo") {
        ...
        if (finalFile.size < targetBytes) {
          finalFile = new File(
            [finalFile, new Uint8Array(targetBytes - finalFile.size)],
            finalFile.name,
            { type: finalFile.type },
          );
        }
      ...
      if (key === "photo") {
        ...
        compliance = {
          ...
          size:
            finalFile.size >= cfg.minSizeKB * 1024 &&
            finalFile.size <= cfg.maxSizeKB * 1024,
          dpi:
            dpi?.x === COMPEX_PHOTO_SPEC.dpi &&
            dpi?.y === COMPEX_PHOTO_SPEC.dpi,
          dimensions:
        ...
        if (!Object.values(compliance).every(Boolean)) {
          throw new Error(
            "The processed photo did not meet all COMPEX requirements.",
          );
        }
      }
  ...
  const allProcessed = Object.values(documents).some(
    (d) => d.processedFile !== null,
  );
              ...
              <div className="w-full md:w-1/2 flex flex-col justify-between">
                <div>
                  <div className="flex items-start gap-2 mb-4">
                    ...
                    <div className="group relative flex items-center justify-center shrink-0 mt-0.5">
                      <svg
                        className="w-5 h-5 text-dark-400 hover:text-white cursor-help transition-colors"
                        fill="currentColor"
                        viewBox="0 0 20 20"
                      >
                        <path
                ...
                {cfg.multi && doc.files.length > 0 && (
                  <p className="text-xs text-dark-500 mt-3">
                    {doc.files.length} image{doc.files.length > 1 ? "s" : ""}{" "}
                    selected — will be merged into a single PDF
                  </p>
                )}
              </div>
                  ...
                  <div className="text-dark-300 flex flex-col items-center justify-center text-center w-full">
                    {cfg.multi ? (
                      <p className="text-sm text-primary-400">
                        {doc.files.length} image
                        {doc.files.length > 1 ? "s" : ""} ready — click Convert
                      </p>
                    ) : (
                      <>
                        <p className="text-sm font-bold truncate w-full mb-1">
                          {doc.file?.name}
                        </p>
                  ...
                  <div className="w-full flex flex-col items-center animate-in fade-in zoom-in duration-300">
                    ...
                    {key === "photo" && doc.compliance && (
                      <div className="w-full mb-4 rounded-lg border border-green-500/20 bg-green-500/5 px-3 py-2 text-xs text-green-300 text-center">
                        JPEG · {(doc.processedFile.size / 1024).toFixed(1)} KB ·
                        700 × 900 px · 200 DPI · 3.5:4.5 aspect ratio
                      </div>
                    )}
